...
fb_grouped = fb_data.groupby(['epsilon', 'cores']).agg(
    # total_running_time_mean=('running_time_baseline_per_core', 'mean'),
    # total_matching_weight_mean=('matching_weight_baseline_per_core', 'mean'),
    running_time_mean=('running_time', gmean),
    running_time_std=('running_time', gstd),
    matching_weight_mean=('relative_matching_weight',gmean ),
    matching_weight_std=('relative_matching_weight', 'std')
).reset_index()
...
def plot_grouped_data_subplot(ax, grouped_data, title, colors, base_markers):
    ...
    # Assign markers
    marker_map = {
        cores: (base_markers[i * 2 % len(base_markers)],
                base_markers[(i * 2 + 1) % len(base_markers)])
    ...
    for i, cores in enumerate(core_values):
        ...
        # --- LEFT AXIS: Absolute Running Time ---
        ax.plot(
            subset['epsilon'],
            subset['running_time_mean'],
            color=color,
            linestyle='-',
            marker=rt_marker,
            markersize=14,
            linewidth=4,
            label=f'Time p={cores}'
        )

        # ax.fill_between(
        #     subset['epsilon'],
        #     subset['running_time_mean'] - subset['running_time_std'],
        #     subset['running_time_mean'] + subset['running_time_std'],
        #     color=color,
        #     alpha=0.2
        # )

        # --- RIGHT AXIS: Relative Matching Weight ---
        ax2.plot(
            subset['epsilon'],
            subset['matching_weight_mean'],
            color=color,
            linestyle='--',
            marker=mw_marker,
            markersize=14,
            linewidth=4,
            label=f'Weight p={cores}'
        )

        # ax2.fill_between(
        #     subset['epsilon'],
        #     subset['matching_weight_mean'] - subset['matching_weight_std'],
        #     subset['matching_weight_mean'] + subset['matching_weight_std'],
        #     color=color,
        #     alpha=0.2
        # )
...
def plot_grouped_data_subplot_old(ax, grouped_data, title, colors, base_markers):
    core_values = sorted(grouped_data['cores'].unique())
    ...
    ax.set_xlabel(r'$\epsilon$')
    # if title == 'fb':
    ax.set_ylabel('Relative Time/Weight')
    ax.set_xscale('log')
    ax.grid(True, which='major', linestyle='--', alpha=0.5)
    ax.grid(True, which='minor', axis='y', linestyle=':', alpha=0.3)
    ax.set_title(title)
...
def plot_grouped_data_subplot_old2(ax, grouped_data, title, colors, base_markers):
    core_values = sorted(grouped_data['cores'].unique())
...
plot_grouped_data_subplot_old(axes[0], fb_grouped, "fb", colors, base_markers)

# Plot rmat_ data on the second subplot
plot_grouped_data_subplot_old(axes[1], rmat_grouped, "rmat", colors, base_markers)
